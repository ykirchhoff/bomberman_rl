...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, norm=nn.InstanceNorm2d, 
        ...
        for _ in range(num_convs - 1):
            self.convs.append(nn.Sequential(nn.Conv2d(out_channels, out_channels, kernel_size, stride, padding=1),
                                            norm(out_channels, **norm_kwargs)))
...
class MyResNet(nn.Module):
    def __init__(self, in_channels: int, num_actions: int, depth: int, num_base_channels: int, 
        ...
        for d in range(depth):
            ...
            self.downs.append(nn.Sequential(nn.MaxPool2d(2), ResBlock(in_channels, out_channels, kernel_size=3, stride=1, num_convs=blocks_per_layer[d]), 
        ...
        self.final = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(out_channels, num_actions))
