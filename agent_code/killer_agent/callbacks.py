...
def setup(self):
    ...
    self.model = MyResNetBinary(in_channels=6, num_actions=len(ACTIONS), depth=3, num_base_channels=32, num_max_channels=512,
...
def state_to_features(game_state: dict) -> np.array:
    ...
    crate_features = np.where(field==1, 1, 0)
    ...
    features = np.stack([field_features, crate_features, bombs_features, explosion_map, coin_features, agent_features])
